...
Schedule or Log Phone Calls
~~~~~~~~~~~~~~~~~~~~~~~~~~~
We can schedule or log a phone calls in order to keep a historical data to manage the opportunity follow ups efficiently. A good sales manager wants their term members to call, call and call... so after each call salesman can notes of call discussion on log call.

.. image:: images/log-calls.png

Log Calls

.. tip::
	You can also integrate a telephone systems such as ``asterisk`` to get the call recording and attach it along with the log call or schedule call once the call complete.
	
Schedule Meeting
~~~~~~~~~~~~~~~~
We can also schedule a meeting on calendar to manage it efficiently, we can add attends to add same event on their calendar on OpenERP, send invitations through email to registered partners. We can also get the confirmation of acceptance or rejections of the meeting through scheduled meeting in OenERP itself.

.. note::
	You can get the all the activities like log calls, schedule a meetings on opportunity in chatter, so that we can track an opportunity for all activities along with the internal notes of email communications.
